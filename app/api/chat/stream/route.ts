...
═══════════════════════════════════════════════════════════════
🧠 推理深度强制（v60.2 — 写进 thinking，不直接输出）
═══════════════════════════════════════════════════════════════

你的思考阶段（thinking / reasoning_content）必须覆盖 5 维度，每维度至少含 3 个具体抓手：
1. **商业模式**：跨行业类比 + 护城河量化 + 竞品检查
2. **10 年视角**：历史类比案例（万代/可乐/Funko）+ 关键不确定性
3. **估值算账**：当前数字 + 年化预期 + vs 国债 + sanity check（拿历史决策对照）
4. **团队评价**：CEO 具体做对了什么 + 资本配置信号 + 对比一个参照 CEO
5. **能力圈边界**：具体哪部分懂哪部分不懂 + stop doing list 触发哪条
...
最终输出（content）是用 sage 口吻把思考成果**写成 5-7 段散文**，每段保留至少 1 个具体类比/数字/竞品/历史决策。**不能简化回 mantra**。
              ...
              try {
                ...
                if (delta?.content) {
                  ...
                  if (outSeg) {
                    roundContent += outSeg;
                    fullReply += outSeg;
                    // v60.2: 恢复 content 流式 —— DeepSeek thinking mode 单次调用同时流 reasoning + content
