...
function makeTool_searchSagePost(sage: SageData) {
  return async function (query: string, top = 5): Promise<string> {
    ...
    const reranked = await bochaRerank(query, candTexts, Math.min(top, 8));
...
const TOOLS = [
  {
    ...
    function: {
      ...
      description: "⭐ 在你（sage）自己的雪球历史发言里做语义搜索（BM25 + reranker）。当用户问到你过去的具体观点、某只股票你怎么看、某概念你怎么解释时**优先用此工具**。返回最相关的 5 条原文（带日期/赞数/链接）。",
      parameters: { type: "object", properties: { query: { type: "string", description: "语义搜索 query，如『腾讯估值』『泡泡玛特换仓理由』『为什么看好招行』" }, top: { type: "number", description: "返回条数 1-8", default: 5 } }, required: ["query"] },
    },
  },
  {
    type: "function" as const,
...
引用自己历史发言时，**必须**在句子末尾加 \`[原文 N]\` 标号，N = search_sage_post 工具返回的 [N] 序号（如 [1]、[2]、[3]）。

❌ 错误："我 5 月 7 号说过把神华换成泡泡玛特了"
✅ 正确："我 5 月 7 号说过把神华换成泡泡玛特了 [原文 1]"

❌ 错误："4 月 4 号我去 Westfield 看了门店"
✅ 正确："4 月 4 号我去 Westfield 看了门店 [原文 2]"

每个引用日期/具体事件后面跟着 \`[原文 N]\`，前端会自动把这个标号渲染成可点击的卡片，让用户看到原帖来源。
...
>
> 我自己会一直拿，因为我看 10 年。但我不卖也不加，市场不是我的仆人。反正我是这么看的，对错我自己负责。
