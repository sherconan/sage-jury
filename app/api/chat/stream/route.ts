...
能力圈窄不丢人，乱出圈才丢人。` },
            ...
            for (const line of lines) {
              ...
              try {
                ...
                if (delta?.content) {
                  ...
                  if (outSeg) {
                    roundContent += outSeg;
                    // v60: 不再把 tool 阶段的 prose 流给用户。Writer 阶段才是真正给用户看的输出。
                    // 旧 fullReply += outSeg 和 chunk event 都删除 —— 留给 Writer 重新生成
                  }
                }
        ...
        // ═══════════════════════════════════════════════════════════════════════
        // v60: 双 LLM 架构 —— Analyst (低温 JSON 5 维度深度分析) → Writer (高温散文)
        // 顶层设计：思考与表达解耦，强制深度推理，再用 sage 口吻表达
        // ═══════════════════════════════════════════════════════════════════════

        // 收集所有工具结果（Analyst 的输入数据）
        const toolResultsBlock = messages
          .filter(m => m.role === "tool")
          .map((m: any) => `[tool_result]\n${m.content}`)
          .join("\n\n---\n\n");

        controller.enqueue(sse("phase", { name: "analyst", message: `${sage.display} 内心深度分析中...` }));

        // ──── Analyst pass ────
        const analystSys = `你是【${sage.display} 的内心专家分析器】—— 不是表达者，是分析者。

你的唯一任务：基于工具数据 + RAG 召回的真实历史发言 + sage skill 文件，输出深度分析 JSON。

⚠️ 输出要求（每一维必须至少有以下 4 个具体抓手中的 3 个）：
1. **具体类比**："X 跟 Y 一个意思" / "类似当年 Z" / "跟 1990 年代日本万代是一码事"
2. **具体数字 + sanity check**：不是只列 PE，而是算"7 年回本 vs 国债 4% 哪个划算"
3. **具体竞争对手或反例**：不能空泛"竞争激烈"，得说"52TOYS / 名创优品 TOP TOY 为什么没打下来"
4. **段永平真实历史决策**："我 2002 年算的账是 X..." / "我拒绝百度因为 Y..." / "我 2024 年买拼多多 Z..."

✋ **绝对不能写笼统的 mantra**："right business" 后必须跟为什么，"看十年"后必须跟具体推理。

只输出 JSON，不输出散文，不写给用户的回答。

JSON Schema:
{
  "business_essence": {
    "one_sentence": "这门生意一句话讲清，不超过 30 字",
    "cross_industry_analogy": "跨行业类比 + 为什么相似",
    "moat_specifics": "护城河具体是什么，量化",
    "competitor_check": "竞争对手为什么打不过，列具体竞品"
  },
  "ten_year_view": {
    "ten_year_essence": "10 年后这家公司还在吗，凭什么",
    "historical_analogy_case": "类比一个真实历史公司的 10 年（万代/可乐/茅台/Funko 等）",
    "key_uncertainty": "我无法预测的关键变量"
  },
  "valuation_math": {
    "current_numbers": "PE/PB/FCF margin 等当前数字",
    "implied_annual_return": "按当前价 + 增长假设的年化预期，给百分比",
    "vs_treasury_or_alternatives": "vs 10 年国债 X% / vs 苹果茅台等持仓",
    "sanity_check": "拿历史案例对照 (如苹果 2002 我算的账)"
  },
  "team_quality": {
    "ceo_assessment": "对 CEO 的具体评价（不是说他靠谱，是说他做对了什么具体事）",
    "capital_allocation_signal": "资本配置能力的具体信号（回购/分红/收购模式）",
    "comparison_to_reference_ceo": "对比一个正/反例 CEO（库克/丁磊/巴菲特）"
  },
  "circle_boundary": {
    "what_i_understand": "我懂的部分（具体说哪部分）",
    "what_i_dont": "我不懂的部分（具体说为什么不懂）",
    "stop_doing_list_check": "触发了 stop doing list 第几条吗，触发就承认"
  },
  "core_conclusion": "一句话给用户的结论 + 建议（自己拿/慢慢加/不碰/再等）",
  "evidence_quote_ids": [1,2,3]  // 从 RAG 召回的 [原文 N] 序号中，挑出真正支撑分析的，最多 5 个
}

工具数据：
${toolResultsBlock || "（本轮未调用工具）"}

RAG 召回的真实历史发言（带 [原文 N] 序号，可在 evidence_quote_ids 引用）：
${ragCtx}

Sage skill 核心（用于校准风格判断）：
${sageSkillBlock.slice(0, 4000)}`;

        let analystJson: any = null;
        let analystRaw = "";
        try {
          const ar = await fetch(`${LLM_BASE}/chat/completions`, {
            ...
            body: JSON.stringify({
              ...
              messages: [
                ...
                { role: "user", content: `用户问题：${userMsg}\n\n现在做 5 维度深度分析。每维度都要具体不要空泛。直接输出 JSON。` },
              ],
          ...
          if (ar.ok) {
            ...
            analystRaw = d?.choices?.[0]?.message?.content || "";
            try { analystJson = JSON.parse(analystRaw); } catch { analystJson = null; }
          }
        } catch (e: any) {
          console.error("Analyst error:", e?.message);
        }
        controller.enqueue(sse("analyst", { json: analystJson, raw_len: analystRaw.length }));

        // ──── Writer pass (stream) ────
        controller.enqueue(sse("phase", { name: "writer", message: `${sage.display} 落笔中...` }));

        const analystForWriter = analystJson
          ? JSON.stringify(analystJson, null, 2)
          : analystRaw || "（Analyst 失败，请基于工具数据自行综合）";

        const writerSys = `${sageSkillBlock}

═══════════════════════════════════════════════════════════════
🎯 你刚才（内心）做了 5 维度深度分析（见 user msg 中的 JSON）
═══════════════════════════════════════════════════════════════

现在用 ${sage.display} 的口吻把这套分析**写成 5-7 段散文**。

⚠️ **必须保留**（违反即失败）：
- JSON 里每个 \`cross_industry_analogy\` / \`historical_analogy_case\` / \`comparison_to_reference_ceo\` —— 类比必须出现
- 每个 \`implied_annual_return\` / \`sanity_check\` —— 具体数字 + 算账必须保留
- 每个 \`competitor_check\` —— 竞品名字必须提
- \`what_i_understand\` / \`what_i_dont\` —— 边界要写清楚
- \`stop_doing_list_check\` 触发了就承认

❌ 禁止简化成"右生意 + 看十年 + 本分"这种空 mantra
✅ 必须保留所有具体决策、数字、对比 —— 把 JSON 的每个 specific point 都翻译成散文

═══════════════════════════════════════════════════════════════
📎 引用规则（v55 + v60）
═══════════════════════════════════════════════════════════════
- JSON 中 \`evidence_quote_ids\` 字段给了你哪些 [原文 N] 真正支撑这次分析
- 你引用历史发言时，**只能用这些 evidence_quote_ids 里的 N**
...
═══════════════════════════════════════════════════════════════
RAG 引用池（你可以用 \`[原文 N]\` 中的 N，但只用 evidence_quote_ids 列表里的）：
${ragCtx}`;
        ...
        // Writer 失败兜底：直接把 Analyst raw 文本流给用户
        if (!writerOk || !fullReply.trim()) {
          const fallback = analystRaw || `[Writer 失败] ${sage.display} 暂时无法回答，请重试。`;
          fullReply = fallback;
          controller.enqueue(sse("chunk", { delta: fallback }));
        }
